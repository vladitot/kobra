...
class FileListsHelper
{
    ...
    public function getOriginFilesArray(Package $package): array
    {
        ...
        $files = $this->convertOriginFilesToCombinedPathList(
            $package,
            $this->helper->getOriginFilesFromPackage($package->name)
        );
    ...
    public function appendFileWithPackageName(string $destination, string $packageName)
    {
        ...
        $splittedPath = explode($destination, DIRECTORY_SEPARATOR);
        unset($splittedPath[count($splittedPath) - 1]);
        $joinedPath = implode($splittedPath, DIRECTORY_SEPARATOR);

        return $joinedPath . DIRECTORY_SEPARATOR . $pathInfo['filename'] . '-' . $packageName . $pathInfo['extension'];
